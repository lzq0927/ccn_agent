...
// 场景 D/E:流控溯源(物联网注册风暴)
  ...
  G: {
    ...
    skillEvolution: {
      kind: "NEW",
      skillId: "skills/learned/udm_overload_admission",
      skillCn: "UDM 过载→AMF/SMF 协同限流",
      insight: "过载点在 UDM 时,CHR(SST/DNN)+UFDR(SUPI)溯源,按 CPU/消息线性推算减量,在 AMF/SMF 侧限流并回 UE 定时器",
      after: "新增 UDM 过载协同限流 skill,命中率达 0.88",
      nextHitRate: 0.88,
    },
  },
};

// 七场景:A(UPF·工作流)、B(SMF·技能引导)、C(gNB 物联终端群体·自主探索)、D/E(物联网风暴·流控溯源)、F(三层并行·终端类型感知)、G(AI平台故障·UDM过载·AMF/SMF协同限流)
