...
//   轨道 + 刻度点(完成=绿实心 / 当前=靛蓝环 / 未来=空心);
//   每步名称常显于刻度点下方(全貌一目了然);当前步上方浮衬线标号+结论;
//   轮1→轮2 画细虚线 ↻ 分隔;播放头 = 1.5px 靛蓝垂线(rAF 直驱)。
//   按段时长比例布点,播放头与点对齐。
// ============================================================================
...
function StepAxisBase({ scenario, state, playheadRef }: Props) {
  ...
  /** 第 i 步段中点百分比(名称标签居中用) */
  const midPct = useMemo(() => startPct.map((s, i) => s + (segDurs[i] / totalDur / 2) * 100), [startPct, segDurs, totalDur]);
  // 轮1→轮2 分隔位置(不占点位)
  ...
  // 当前步浮标位置:贴当前步段中点,clamp 防溢出
  const labelPct = Math.max(7, Math.min(93, midPct[curIdx] ?? 0));

  return (
    <div className="panel" style={{ padding: "6px 14px 5px", display: "flex", alignItems: "stretch", gap: 12 }}>
      <div style={{ display: "flex", flexDirection: "column", lineHeight: 1.2, flexShrink: 0, minWidth: 64, justifyContent: "center" }}>
      ...
      <div style={{ flex: 1, height: 54, minWidth: 0, position: "relative" }}>
        {/* 当前步浮标:标号 + 结论(衬线步名在各步标签区已有,这里补结论) */}
        <div style={{ position: "absolute", left: `${labelPct}%`, top: 0, transform: "translateX(-50%)", whiteSpace: "nowrap", pointerEvents: "none", transition: "left 0.3s var(--ease-out)" }}>
          <span style={{ fontSize: 10, color: ACCENT }}>
            <span className="mono" style={{ fontSize: 9, marginRight: 4 }}>{String(curIdx + 1).padStart(2, "0")}</span>
            {curBrief}
          </span>
        </div>

        {/* 轨道 */}
        <div style={{ position: "absolute", left: 0, right: 0, top: 26, height: 2, background: "var(--line-2)", borderRadius: 1 }} />

        {/* 各步:刻度点(段起点) + 名称标签(段中点,常显)—— 直接相对轨道容器定位 */}
        {stops.map((stop, i) => {
          const status: "done" | "cur" | "future" = i < curIdx ? "done" : i === curIdx ? "cur" : "future";
          const ph = PHASES[stop.phase];
          const isR2 = stop.round === 2;
          const tip = `${i + 1}. ${ph.cn}${isR2 ? " · 第②轮" : ""} — ${stepBrief(scenario, stop.phase, stop.round, twoRound)}`;
          return (
            <Fragment key={`${stop.phase}-${stop.round}-${i}`}>
              ...
              <div
                ...
                style={{
                  ...
                  color: status === "cur" ? "var(--ink-1)" : status === "done" ? "var(--ink-3)" : "var(--ink-5)",
                  transition: "color 0.25s ease",
                }}
              >
                {SHORT_LABEL[stop.phase]}
                {isR2 && <span style={{ fontSize: 7.5, color: WARN, marginLeft: 2, fontWeight: 600 }}>R2</span>}
              </div>
            </Fragment>
          );
        })}
        ...
        {/* 轮1 → 轮2 回路分隔(细虚线 + ↻) */}
        {boundaryPct !== null && (
          <div title="loop② 回 Agent1" style={{ position: "absolute", top: 18, bottom: 4, left: `${boundaryPct}%`, width: 0, borderLeft: "1px dashed " + WARN + "99", pointerEvents: "none" }}>
            <span style={{ position: "absolute", top: -2, left: -4.5, fontSize: 10, color: WARN }}>↻</span>
          </div>
        )}

        {/* 播放头:细垂线 + 顶端小三角(rAF 直驱 left) */}
        <div ref={playheadRef} style={{ position: "absolute", top: 16, bottom: 4, left: 0, width: 1.5, background: ACCENT, pointerEvents: "none", zIndex: 4 }}>
