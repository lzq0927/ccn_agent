...
export interface ClockApi {
  ...
  /** 从当前时刻播放到 target(绝对秒)后自动暂停;target≤当前则瞬时定位 */
  playUntil: (target: number) => void;
}

export function useStoryClock(scenario: Scenario): ClockApi {
  const LOOP_DURATION = loopDurationFor(scenario);
  const [time, setTime] = useState(0);
  const [playing, setPlaying] = useState(false); // 交互式:默认不自动播放,由点击驱动
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (playingRef.current) {
        ...
        // playUntil:到达目标时刻即暂停(点圆圈只播到该步结束,不循环不越界)
        if (playUntilRef.current != null && nt >= playUntilRef.current) {
          nt = playUntilRef.current;
          playUntilRef.current = null;
          playingRef.current = false;
          setPlaying(false);
        }
        if (playUntilRef.current == null && nt >= dur) {
          nt -= dur;
          loopRef.current += 1;
  ...
  /** 从当前时刻播放到 target 后自动暂停(点圆圈只播到该步结束) */
  const playUntil = (target: number) => {
    if (target <= timeRef.current + 0.01) {
      // 目标在当前或之前 → 瞬时定位
