...
const R = 26; // 节点半径(引导式舞台放大,便于展会远观)
...
interface Props {
  ...
  /** true = 极简模式(引导式舞台用):隐藏 NRF 注册线 + 左下健康摘要,突出业务流主干。 */
  minimal?: boolean;
  /** true = 引导交互模式:业务流边淡化为静态底纹(opacity 0.1)、隐藏 UE 簇,
   *  让叠加的采集/下发箭头清晰不乱。 */
  guide?: boolean;
}
...
function cpuText(cpu: number, radius: number, type: string) {
  ...
  return <text y={radius + 36} textAnchor="middle" fontSize={13} fontWeight={700} fill={c} fontFamily="var(--font-mono)">CPU {cpu}%</text>;
}

function DigitalTwinBase({ scenario, state, graph, kpi: kpiProp, showCallouts = true, minimal = false, guide = false }: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
      ...
      {/* 业务流链路 —— guide 模式:淡化为静态底纹,突出叠加的采集/下发箭头 */}
      <g>
        {g.flowEdges.map((e) => {
          ...
          if (guide) {
            // 引导模式:全部静态淡底纹,只有劣化/重路由边略强调
            const gColor = isReroute ? STATUS.recovered : degraded ? srColor(sr) : baseColor;
            const gOp = isReroute ? 0.9 : degraded ? 0.55 : 0.1;
            const gW = isReroute ? 3.2 : degraded ? 1.6 : 0.9;
            return <line key={e.id} x1={ll.x1} y1={ll.y1} x2={ll.x2} y2={ll.y2} stroke={gColor} strokeWidth={gW} strokeLinecap="round" opacity={gOp} />;
          }
        ...
        {g.nodes.map((n) => {
          ...
          return (
            <g key={n.id} transform={`translate(${n.x} ${n.y})`}>
              ...
              <circle r={R} fill={fillUrl} stroke={ringColor} strokeWidth={isFocus || isRoot ? 3 : 2} filter={isFocus || isRoot || degraded ? "url(#twin-glow)" : undefined} />
              <text y={7} textAnchor="middle" fontSize={22} fontWeight={700} fill={isCordoned ? "var(--text-mid)" : `var(--ne-type-fill, ${tc.glow})`} fontFamily="var(--font-mono)">
                {n.type}
              </text>
              <text y={R + 20} textAnchor="middle" fontSize={17} fontWeight={700} fill={isRoot ? (isUserFaultGnb ? STATUS.warning : STATUS.faultGlow) : isCordoned ? "var(--text-faint)" : "var(--text-mid)"} fontFamily="var(--font-mono)">
                {n.id}
              </text>
              {/* SIM 模式:全网 CPU% 显示 */}
              {state.simNeCpu && state.simNeCpu[n.id] != null && cpuText(state.simNeCpu[n.id], R, n.type)}
              {degraded && !isUserFaultGnb && (
                <text y={-R - 12} textAnchor="middle" fontSize={14} fontWeight={700} fill={STATUS.faultGlow} fontFamily="var(--font-mono)">
                  {(sr * 100).toFixed(1)}%
                </text>
              )}
              {n.role !== "lb" && (
