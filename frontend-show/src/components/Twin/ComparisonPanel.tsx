  ...
  return (
    <div className="hud" style={{ borderRadius: 10, padding: "7px 10px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <span style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5f6f87", fontFamily: "var(--font-mono)" }}>诊断对比</span>
        <span style={{ fontSize: 8.5, color: "#475569", fontFamily: "var(--font-mono)" }}>NAIVE NETWORK VIEW ↔ MULTI-DIM EXPLORATION</span>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "stretch" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 64, opacity: 0.4 + 0.6 * rev }}>
          <div style={{ fontSize: 8.5, letterSpacing: "0.08em", color: "#5f6f87", fontFamily: "var(--font-mono)", marginBottom: 3 }}>多维探索</div>
          ...
          <div style={{ fontSize: 8, color: STATUS.healthy, fontFamily: "var(--font-mono)", marginTop: 2 }}>{rev > 0.5 ? "已收敛" : "探索中"}</div>
        </div>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
        <span style={{ fontSize: 8.5, letterSpacing: "0.08em", color: "#5f6f87", fontFamily: "var(--font-mono)" }}>{title}</span>
        <span style={{ marginLeft: "auto", fontSize: 8, padding: "1px 6px", borderRadius: 3, color, border: `1px solid ${color}66`, fontFamily: "var(--font-mono)" }}>
          {done ? "✓ " : ""}
          {tag}
        </span>
      </div>
      <div style={{ fontSize: 11.5, fontWeight: 700, color: done || !dim ? "#eaf4ff" : "#9fb0c9", lineHeight: 1.25 }}>{verdict}</div>
      <div style={{ fontSize: 9, color: "#8a9bb5", marginTop: 3, lineHeight: 1.4 }}>{detail}</div>
    </div>
  );
}
