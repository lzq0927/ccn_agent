import random
from .models import Topology, DC, ResourcePool, NetworkElement, NEType, MASTER_STANDBY_TYPES
...
class TopologyGenerator:
    def generate(self, config_index: int, seed=None):
        ...
        for ne_type, (min_cnt, max_cnt) in config["ne_counts"].items():
            ...
            if ne_type in MASTER_STANDBY_TYPES:
                self._gen_master_standby(
                    ne_type, count, pool_info, total_pools, elements, ne_counters
                )
            else:
                for _ in range(count):
                    ...
                    elements[ne_id] = NetworkElement(
                        id=ne_id, ne_type=ne_type, pool_id=pool_id, dc_id=dc_id, role="lb"
                    )
        ...
        return Topology(dcs=list(dc_map.values()), elements=elements, switches=switches)

    def _gen_master_standby(self, ne_type, count, pool_info, total_pools, elements, ne_counters):
        ...
        for i, role in enumerate(["master", "standby"]):
            ...
            ne_counters[ne_type] += 1
            elements[ne_id] = NetworkElement(
                id=ne_id, ne_type=ne_type, pool_id=pool_id, dc_id=dc_id, role=role
            )
